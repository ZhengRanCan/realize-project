...
### 3.2 Relation gap = **D 6 · E 2** → **对 D 不可控**（**修复后 D = 2**，见 §7）
...
**False Negative = 1 处（**修复前**；已修复并复测，见 §7）** ⚠️
...
## 5. Gate（修复前判定 —— **已被 §7 取代**）

> ⚠️ **本节记录的是修复轮的输入，不是最终判定。** 修复轮（R1–R8）已执行完毕，
> 最终 Gate 见 **§7**（`Gate = PASS`）。
...
依据用户裁决执行的修复（详见 `repair-round.md`）。逐项复测：

| 项 | 修复前 | 修复后 | 判定 |
|---|---|---|---|
| D 的 `N2 / N3` | 未执行（`W0` + `SKIPPED`） | **已执行**：`coverage 21/21`，`SKIPPED 0` | ✅ 覆盖缺口关闭 |
| D 的状态 | `PASS WITH INCOMPLETE VALIDATION` | **`PASS`** | ✅ |
| **M8 在 D 上** | ❌ 漏网 | ✅ **拦住** → 拦截率 **14/14 = 100%** | ✅ |
| D 的 relationGap | 6（不可控） | **2**（缺口密度 0.15） | ✅ 可控 |
| D 的 edges | 9 | 11（+ `contains`+`reference`、`relates-to`+`shared`） | ✅ 关系层能表达了 |
| `W4` | Warning × 3（跨 B / E） | **INFO（I6）** | ✅ 降级 |
| `W5` 报告形态 | 6 条平铺 | 少而散逐条；多而密聚合为 `W8` + detail | ✅ 噪音消除 |
| E 的基线 | HARD 0 · WARN 5 · INFO 5 | HARD 0 · **WARN 3 · INFO 7**（`W4`×2 → `I6`） | ✅ 仅降级影响 |
| 五篇 Fixture | — | **A / B / C / D / E 全部 HARD 0 · `PASS`** | ✅ |
| 单测 | 21/21 | **29/29**（新增 heading tree / qualifiers / 聚合用例） | ✅ |
...
| # | 条件 | 判定 | 依据 |
|---|---|---|---|
| 1 | Semantic gap = 0 | ✅ | D / E 均不需要第 7 类 element |
| 2 | Relation gap 可控 | ✅ | D 6 → 2；基础语义 + 结构属性已能表达 5 类的原本缺口；剩余 2 类是真第 3 层（Constraint） |
| 3 | Capacity 只是 heuristic | ✅ | E = 13 触发 `W1`；预算保持 12 不变，"明显不够"未被证明 |
| 4 | Validator 无明显误报 | ✅ | FP = 0；**FN 1 处已修复并复测**（M8 在 D 上被拦住） |

**Gate 条件 4 的措辞必须保留这句边界（不得读成"关系模型完备"）：**
...
1. **B / C 的 map 尚未用 `qualifiers` 重表达** —— §5.2 放宽 `contains` 后，C 的"持有 / 存储"缺口应可消除，列为 follow-up。
2. **`constraint.parameters` 不存在** —— 第 3 层只有元素位置，没有结构化参数位。
3. **v2 的 D / E 仍是工程验证，不是独立选型** —— 候选池在此前已被观察过；更强的证据需要外部 holdout。
4. **语义验收标准仍是 proxy** —— "语义覆盖"目前用 1:1 语义 proxy 度量，不是真正的语义判据。

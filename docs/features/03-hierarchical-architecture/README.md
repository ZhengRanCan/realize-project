...
两区再加上文档级入口，构成三个**并列**的东西，各回答一个问题：
...
> ⚠️ **不要要求 L0 去"证明自己不是流程图"。**
> 如果一篇文档的核心就是执行流程，它的 Framework Map 长得像流程图是**完全正常**的（Feature 05 实测：Process-heavy 的 Fixture C 就是如此）。
> 真正不能退化的是**整个产品**只有流程图 —— 只要有 Topic / L1 / L2 承载其余内容（confidence、完成语义、生命周期、可观测性、rollout、open questions），它就仍然是 **Document Model**。
...
2. **Process-heavy 文档的机制链会不会与 topic 划分几乎重合**，导致 L0 退化成一张普通流程图（那就失去了"地图"的意义）？
   ...
   > **Feature 05 已裁定：这不是风险。** L0 长得像流程图是正常形态；真正不能退化的是整个产品只有流程图。见 §3.1。
...
| 元素选不准 | 判据 A~F + 元素总数软预算 + Phase 1 手工验证 |
| 元素没有溯源，图变成"AI 插画" | **硬约束**：每个元素必须有 `sourceUnitIds`（或 B/C 类的 `sectionRefs`）。没有 provenance 的图是**最容易丢语义**的地方 —— 丢了你都发现不了 |
| topic 划分不准确 | topic 已降级为标签，影响面大幅缩小；改用五判据衡量 |
| **整个产品退化成 flowchart viewer**（只有 L0，没有 Topic / L1 / L2 承载其余内容） | 保住三种 coverage 的分离（§7）与 Topic 入口（§7.2）。<br>⚠️ **L0 自身长得像流程图不是风险** —— 这条已在 Feature 05 撤销（见 §3.1） |
| 人工对比多篇文档图成本高 | 明确不在本特征内；v1 靠 `label` 规范性 |
| 复核时无法判断图上是否漏了重要内容 | §7.2 的 Navigation invariant（N1~N3）可自动 check |
